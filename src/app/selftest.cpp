...
CheckResult pass(QString name, QString detail) {
    return {std::move(name), true, std::move(detail), {}};
}

CheckResult fail(QString name, QString detail, QString remedy) {
    return {std::move(name), false, std::move(detail), std::move(remedy)};
}
...
CheckResult checkWebSockets() {
    ...
    QWebSocketServer probe(QStringLiteral("dstdesk-selftest"),
    ...
    if (!QSslSocket::supportsSsl()) {
        return fail(
            ...
            QStringLiteral("Qt bundles no OpenSSL and loads the system one at "
                           "runtime. The OpenSSL that Conan provides must be on the "
                           "library path — run the app through `conanrun` or the "
                           "packaged launcher. See decision 12 in dstOMNI/DESIGN.md."));
    }
...
CheckResult checkPort(std::uint16_t port) {
    ...
    if (!probe.listen(QHostAddress::LocalHost, port)) {
        return fail(QStringLiteral("Configured port"),
                    ...
                    QStringLiteral("Another process is using the port. Stop it, or "
                                   "set a different port in the target config."));
    }
    probe.close();
...
CheckResult checkOutputDir(const QString& dir) {
    ...
    const QString probePath = d.filePath(QStringLiteral(".dstdesk-write-test"));
    ...
    return pass(QStringLiteral("Output directory"), QDir::toNativeSeparators(d.path()));
}
...
CheckResult reportProtocol() {
    ...
    return pass(QStringLiteral("Protocol"),
                QStringLiteral("v%1, %2 Hz, %3 samples/frame, %4 B/frame")
                    .arg(protocol::kVersion)
                    .arg(protocol::kSampleRate)
                    .arg(protocol::kFrameSamples)
                    .arg(protocol::kFrameBytes));
}
...
int printSelfTest(const SelfTestReport& report) {
    ...
    for (const auto& c : report.checks) {
        out << (c.passed ? "  ok    " : "  FAIL  ") << c.name.leftJustified(20)
            << c.detail << Qt::endl;
    }
    out << Qt::endl;
